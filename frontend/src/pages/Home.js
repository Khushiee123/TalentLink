...
import { Bell, MessageSquare, FileText, CheckCircle, Briefcase, Star } from 'lucide-react'; // Added Star icon
...
// --- 1. NotificationBell Component (Unchanged from your snippet) ---
...
export default function Home() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const [profileRes, projectRes, propRes, contRes, reviewRes] = await Promise.all([
        ...
        API.get("/proposals/"), 
        API.get("/contracts/"),
        API.get("/reviews/") // Ensure your backend has this endpoint
      ]);
  ...
  // Shared Star Component for the Reviews Tab
      ...
      <aside className={`sidebar ${isSidebarCollapsed ? "collapsed" : ""}`}>
        ...
        <nav className="side-nav">
          {[
            ...
            { id: 'reviews', label: 'Reviews', icon: <Star size={18}/> } // NEW REVIEWS OPTION
          ].map(tab => (
            <button 
              key={tab.id}
